...
gt <- global.rtest(xpop$tab, spca$lw, k = 1, nperm = 99)
plot(gt)
lt <- global.rtest(xpop$tab, spca$lw, k = 1, nperm = 99)
plot(lt)
screeplot(spca)
...
for (i.axis in 1: 40) {
    ...
    data <- data.frame(spca=spca$li[,i.axis], X=Diplodus_coord_Albers[,1], Y=Diplodus_coord_Albers[,2])
    # 0 - Null model
    ...
    nnmsk[[i.axis]] <- mask(nn,filter(pus, Mullus_surmuletus==1))$var1.pred
}
rmse_nn
which(perf_nn>0)
Mullus_spca <- rast(nnmsk[which(perf_nn>0)])
writeRaster(Mullus_spca, file="Results/Mullus_allAxes_2753.grd")
